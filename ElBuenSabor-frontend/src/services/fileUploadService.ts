 ...
 * Necesaria aquí para construir las URLs de las imágenes.
 */
 ...
 */
export const uploadFile = async (file: File, token: string, articuloId?: number, promocionId?: number): Promise<any> => {
  setAuthToken(token);
 ...
 */
export const deleteFileFromServer = async (filename: string, token: string): Promise<void> => {
  setAuthToken(token);
